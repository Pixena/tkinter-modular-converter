...
class LengthTab:
    def __init__(self, master):
        ...
        title = ttk.Label(self.frame, text="Конвертер длины", font=('Arial', 14, 'bold'), foreground=ACCENT_COLOR)
        title.pack(pady=(10, 2))

        desc = ttk.Label(self.frame, text="Введите значение, выберите единицы и нажмите 'Перевести'.", font=('Arial', 10), foreground="#bbb")
        desc.pack(pady=(0, 10))
        ...
        self.from_var = tk.StringVar(value=config.LENGTH_UNITS[0])
        self.from_menu = ttk.Combobox(units_frame, textvariable=self.from_var, values=config.LENGTH_UNITS, width=12, state="readonly")
        self.from_menu.pack(side=tk.LEFT, padx=5)
        ttk.Label(units_frame, text="в:").pack(side=tk.LEFT)
        self.to_var = tk.StringVar(value=config.LENGTH_UNITS[1])
        self.to_menu = ttk.Combobox(units_frame, textvariable=self.to_var, values=config.LENGTH_UNITS, width=12, state="readonly")
    ...
    def convert(self):
        ...
        result = length_logic.convert_length(num, from_unit, to_unit)
